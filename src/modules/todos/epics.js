...
const fetchTodos = action$ =>
  action$.ofType(TODOS_FETCH_REQUEST)
    .mergeMap(() =>
        ajax.getJSON('https://sql-node-api.herokuapp.com/todo/api/v1.0/tasks')
...
const addTodo = action$ =>
  action$.ofType(TODOS_ADD_REQUEST)
    .mergeMap(action =>
      ajax.post('https://sql-node-api.herokuapp.com/todo/api/v1.0/tasks', {
          title: action.text,
...
const removeTodo = action$ =>
  action$.ofType(TODOS_REMOVE_REQUEST)
    .mergeMap(action =>
      ajax.delete(`https://sql-node-api.herokuapp.com/todo/api/v1.0/tasks/${action.id}`)
        .map(() => ({ type: TODOS_REMOVE_SUCCESS, _id: action.id }))
        .catch(console.log(`Failed to remove task #${action.id}`)),
    );

const completeTodo = action$ =>
  action$.ofType(TODOS_COMPLETE_REQUEST)
    .mergeMap(action =>
      ajax.put(`https://sql-node-api.herokuapp.com/todo/api/v1.0/tasks/${action.id}`, {
